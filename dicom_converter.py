...
def getMetadata(dataset:dcm.FileDataset) -> tuple[str, str]:
...
def convertExam(examDir:str, idx:int, sliceID:int, pngPath:str, pklPath:str) -> dict:
    metaData = {}
...
def main(examDir:str, pngPath:str=None, pklPath:str=None, sliceID:int=20):
    ...
    for idx, examFolder in enumerate(os.scandir(examDir)):
        if examFolder.name.startswith("DBT-P"):
            ...
            metaData.append(convertExam(examFolder.path, idx, sliceID, pngPath, pklPath))
