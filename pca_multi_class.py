...
checkpoint_path = "/home/ubuntu/gmic/trained_models/multi-class-models/epoch=9-step=2900.ckpt"
config_path = "src/config.toml"

device = "cuda" if torch.cuda.is_available() else "cpu"
use_tsne = False  # Set to True for t-SNE

# Optional: Update with your real class names
CLASS_NAMES = ["Mass", "Calcification", "Asymmetry", "Focal Asymmetry", "Arch Distortion", "No Lesion"]
...
def extract_label(batch_label):
    """
    Automatically handle one-hot vs class index labels.
    """
    if batch_label.ndim > 0 and batch_label.shape[0] > 1:
        return torch.argmax(batch_label).item()  # one-hot
    else:
        return batch_label.item()  # already class index
...
def main():
        ...
        for batch_idx, (images, batch_labels, batch_paths) in enumerate(pbar):
            for i in range(len(batch_paths)):
                ...
                try:
                    ...
                    labels.append(extract_label(batch_labels[i]))  # multi-class aware
    ...
    print("Saving PCA arrays...")
    np.save("pca_features.npy", reduced_features)
    np.save("pca_labels.npy", labels)
    ...
    # Set class names on colorbar if CLASS_NAMES match num_classes
    if len(CLASS_NAMES) == parameters["num_classes"]:
        ticks = np.arange(len(CLASS_NAMES))
        colorbar = plt.colorbar(scatter, ticks=ticks)
    ...
    output_file = f"{'tsne' if use_tsne else 'pca'}_gmic_features_multiclass.png"
