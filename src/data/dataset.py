import numpy as np
import pandas as pd
import os, ast, sys, albumentations as alb, albumentations.pytorch as alp
...
class ClassificationImages(Dataset):
    def __init__(self, data_dir: str):
        ...
        self.images = [[entry.path for entry in os.scandir(os.path.join(data_dir, f'{i}'))] for i in range(len(labels))]
        self.labels = [(label, len(self.images[i])) for i, label in enumerate(labels)]

        self.offsets = [sum(len(category) for category in self.images[:i]) for i in range(len(self.labels) + 1)]
        self.class_weights = [self.offsets[-1] / len(self.labels) / size for _, size in self.labels]
    ...
    def __getitem__(self, index: int):
        ...
        x = loading.read_image_standardized(self.images[label][index - self.offsets[label]])
